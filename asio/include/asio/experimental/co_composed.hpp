...
/// Obtain an initiation function object that may be used to launch a
/// coroutine-based composed operation.
/**
 * The experimental::composed utility simplifies the implementation of composed
 * asynchronous operations by automatically wrapping a coroutine with a
 ...
 * that use experimental::composed. It must be explicitly enabled by calling the
 * state's @c reset_cancellation_state function.
 *
 * @par Examples
 ...
 *       asio::experimental::co_composed(socket), token,
 ...
 * coroutine via @c co_return. Valid @c co_return values are specified in the
 * coroutine function's return type, @c asio::experimental::co_returns.
 *
 * @code template <typename CompletionToken>
 * auto async_echo(tcp::socket& socket,
 *     CompletionToken&& token)
 ...
 *     CompletionToken, void(std::error_code)>(
 *       asio::experimental::co_composed(socket), token,
 *       [](auto state, tcp::socket& socket)
 *         -> asio::experimental::co_returns<
 *             void(std::error_code)>
 *       {
 *         try
 *         {
...
auto co_composed(IoObjectsOrExecutors&&... io_objects_or_executors);
